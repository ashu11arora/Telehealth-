...
const AppointmentForm = ({ doctor, onSubmit, onCancel, onPay }) => {
  ...
  const HandleClick = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:8080/order', data).then(res => {
    ...
    <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50 bg-opacity-10 backdrop-blur-xs">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md bg-opacity-20 backdrop-blur-3xl">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <p className="text-sm ">You have to pay indian rupees 500 to book the appointment</p>
          <p className="text-blue-500 text-sm">Click on pay button</p>
          <br></br>
          <div className="text-center mb-4">
            <button
              type="button"
              onClick={HandleClick}
              className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg focus:outline-none focus:shadow-outline-green active:bg-green-800 transform hover:scale-105 transition-transform duration-300 ease-in-out"
            >
